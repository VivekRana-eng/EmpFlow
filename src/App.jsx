...
const navItems = [
  ...
  { label: 'Onboarding', icon: UserPlus, count: 12 },
...
function App() {
  ...
  return (
    <div className="min-h-screen bg-[#e9e8e1] p-3 text-slate-900 sm:p-6 lg:p-8">
      <aside className={`fixed bottom-8 left-8 top-8 z-30 hidden rounded-[22px] border border-white/80 bg-white shadow-[0_12px_30px_rgba(53,65,59,0.08)] transition-all duration-200 lg:flex lg:flex-col ${collapsed ? 'w-[68px]' : 'w-[232px]'}`}>
      ...
      <main className={`w-full transition-all duration-200 ${collapsed ? 'lg:pl-[84px]' : 'lg:pl-[248px]'}`}>
        <header className="sticky top-0 z-20 flex h-[76px] items-center justify-between border-b-0 bg-transparent px-2 sm:px-5">
          <div className="flex items-center gap-3"><button className="rounded-lg p-2 text-slate-500 hover:bg-slate-50 lg:hidden"><Menu size={20} /></button><div><div className="flex items-center gap-2 text-xs text-slate-400"><span>Workspace</span><ChevronRight size={13} /><span className="font-medium text-slate-600">{activePage}</span></div><h1 className="mt-0.5 text-[17px] font-bold tracking-tight text-slate-900">{activePage}</h1></div></div>
